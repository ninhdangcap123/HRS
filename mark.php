<?php
	...
	if(isset($_GET["rno"]))
	{
		$sql="select * from trainee where RNO='{$_GET["rno"]}'";
		$res=$db->query($sql);
		if($res->num_rows<=0)
		{
			...
			<div id="section">
				...
				<div class="content">
					...
					<?php
						...
						{
							$sq="insert into score(REGNO,SUB,MARK,TERM,CLASS) values ('{$_POST["regno"]}','{$_POST["sub"]}','{$_POST["mark"]}','{$_POST["etype"]}','{$_POST["class"]}')";
							if($db->query($sq))
							{
					...
					<form method="post" action="<?php echo $_SERVER["REQUEST_URI"];?>">
						...
						<label>Course</label><br>
							...
							<br><br>
							<button type="submit" style="float:right;" class="btn" name="submit"> Add Score Details</button>
					</form>							
						</div>
