...
<html>
	...
	<body>
		<?php include"navbar.php";?><br>
			...
			<div id="section">
				...
				<div class="content">
					...
					<?php
						if(isset($_POST["submit"]))
						{
							...
							{
								$sq="insert into trainee(RNO,NAME,FNAME,DOB,GEN,PHO,MAIL,ADDR,SCLASS,SSEC,SIMG,TID,TrPass) values('{$_POST["rno"]}','{$_POST["name"]}','{$_POST["fname"]}','{$edate}','{$_POST["gen"]}','{$_POST["pho"]}','{$_POST["email"]}','{$_POST["addr"]}','{$_POST["cla"]}','{$_POST["sec"]}','{$target_file}','{$_SESSION["TID"]}',1234)";
				...
				<div class="lbox">
					<label> ID</label><br>
						<?php
							$no="S101";
							$sql="select * from trainee order by ID desc limit 1";
							$res=$db->query($sql);
							if($res->num_rows>0)
							{
				...
				<label> Relative's Mail </label><br>
					...
					<select name="cla" required class="input3">
						...
						<?php 
							 $sl="SELECT DISTINCT(CNAME) FROM category";
							$r=$db->query($sl);
								if($r->num_rows>0)
									{
					...
					<br><br>
						...
						<?php 
							 $sl="SELECT DISTINCT(CSEC) FROM category";
							$r=$db->query($sl);
								if($r->num_rows>0)
									{
										echo"<option value=''>Select</option>";
			...
			<button type="submit" style="float:right;" class="btn" name="submit">Add Trainee Details</button>
				</div>
